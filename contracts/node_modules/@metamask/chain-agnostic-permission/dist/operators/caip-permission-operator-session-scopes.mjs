 ...
 * @returns A NormalizedScopesObject.
 */
export const getSessionScopes = (caip25CaveatValue, { getNonEvmSupportedMethods, sortAccountIdsByLastSelected, }) => {
    const mergedScopes = mergeNormalizedScopes(getNormalizedScopesObject(caip25CaveatValue.requiredScopes, {
    ...
    if (sortAccountIdsByLastSelected) {
        Object.keys(mergedScopes).forEach((scopeString) => {
            ...
            if (scopeObject) {
                scopeObject.accounts = sortAccountIdsByLastSelected(scopeObject.accounts);
            }
        });
    }
    return mergedScopes;
...
//# sourceMappingURL=caip-permission-operator-session-scopes.mjs.map